...
for doc_id, filename in enumerate(files, start=1):
    ...
    if filename.endswith('.txt'):
        ...
        doc_names[doc_id] = filename #nama file
...
def boolean_retrieval(term):
    ...
    if stemmed_term in inverted_index:
        return [doc_names[doc_id] for doc_id in sorted(list(inverted_index[stemmed_term].keys()))]  # Kembalikan dokumen yang mengandung term
    else:
        return []
...
if result:
    ...
    print(f"Ditemukan di dokumen: {result_str}")
else:
